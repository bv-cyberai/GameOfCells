...
public class MainMenuScreen implements GameOfCellsScreen {
    ...
    public void handleInput(float deltaTimeSeconds) {
        ...
        if (inputProvider.isKeyJustPressed(Input.Keys.W) | inputProvider.isKeyJustPressed(Input.Keys.UP)) {
            selectedOption = (selectedOption - 1 + MENU_OPTIONS.length) % MENU_OPTIONS.length;
        }
        if (inputProvider.isKeyJustPressed(Input.Keys.S) | inputProvider.isKeyJustPressed(Input.Keys.DOWN)) {
