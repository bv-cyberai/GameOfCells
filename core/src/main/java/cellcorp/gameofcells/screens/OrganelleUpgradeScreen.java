...
public class OrganelleUpgradeScreen implements GameOfCellsScreen {
    ...
    private void createUI() {
        menuSystem.initialize("Organelle Upgrades", new String[] {}, "");
    ...
    private Table createUpgradeCard(OrganelleUpgrade upgrade) {
        ...
        if (upgradeIcon != null) {
            ...
            card.add(upgradeIcon).width(100).height(100).padTop(30).padBottom(10).row();
        }
    ...
    private void checkAllUpgradesPurchased() {
        if (upgrades.isEmpty()) {
            ...
            comingSoonLabel.setPosition(ShopScreen.VIEW_RECT_WIDTH / 2 - comingSoonLabel.getWidth() / 2, ShopScreen.VIEW_RECT_HEIGHT / 2 - comingSoonLabel.getHeight() / 2); // Centered
    ...
    public void handleInput(float deltaTimeSeconds) {
        if(inputProvider.isKeyJustPressed(Input.Keys.LEFT) || inputProvider.isKeyJustPressed(Input.Keys.A)) {
            if (selectedUpgradeIndex > 0) {
                selectedUpgradeIndex--;
                updateUpgradeSelection();
            }
        } else if(inputProvider.isKeyJustPressed(Input.Keys.RIGHT) || inputProvider.isKeyJustPressed(Input.Keys.D)) {
    ...
    /**
     * Get the list of organelle upgrades.
     * @return The list of organelle upgrades.
     */
